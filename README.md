...
| **XGBoost** | ❌ Not Used | More complex to implement and tune for this project. |
| **Linear Regression** | ❌ Not Used | Designed for regression problems, not binary classification tasks. |
...
Among all the considered Machine Learning models, **Logistic Regression** was selected because it provides a good balance of **accuracy, simplicity, interpretability, and fast training time**. The model achieved approximately **77% accuracy** on the **Kaggle Pima Indians Diabetes Dataset (768 patient records)**, making it suitable for this diabetes prediction system.
